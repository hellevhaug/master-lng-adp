...
def read_global_data_RHH(group, filename):
    ...
    charter_vessel_lower_capacity, loading_to_time, des_loading_ports, fob_loading_ports, maintenance_start_times
...
def init_model_vars_RHH(model, prediction_horizon, horizon_length, fob_ids, fob_days, 
    ...
    production_quantities = {(i,t): value for (i,t), value in production_quantities.items()}
...
def relax_horizon(model, prediction_horizon, overlap, horizon_length, iteration_count):
    ...
    if prediction_horizon == "ALL": 
        ...
        for var in model.getVars(): 
            if var.varName[0] == 'x':
                ...
                if  horizon_length*(iteration_count+1)+overlap <= int(varName_list[2]) <= horizon_length*(iteration_count+1)+prediction_horizon:
                    var.setAttr("VType", GRB.CONTINUOUS)
                ...
                if  horizon_length*(iteration_count+1)+overlap <= int(varName_list[1]) <= horizon_length*(iteration_count+1)+prediction_horizon:
                    var.setAttr("VType", GRB.CONTINUOUS)
    ...
    model.update()
    print("\n--- Done relaxing x, z, w constraints in: %.1f seconds ---" % (time.time() - start_time))
...
def init_objective_and_constraints(model, x, z, w, g, s, horizon_length, prediction_horizon, \
    ...
    if (horizon_length*(iteration_count+1)+prediction_horizon) >= len(loading_days)-(prediction_horizon/2) and (horizon_length*(iteration_count+1)+prediction_horizon) < len(loading_days):
        horizon_ext = len(loading_days)-(horizon_length*(iteration_count+1))+1
    else:
        horizon_ext = 0
     
    stop_time = horizon_length*(iteration_count+1)+prediction_horizon if horizon_length*(iteration_count+1)+prediction_horizon <= len(loading_days) else len(all_days)
    leftover_stop_time = horizon_length*(iteration_count+1)+prediction_horizon if horizon_length*(iteration_count+1)+prediction_horizon <= len(loading_days) else len(loading_days)
...
def remove_variables_after_pred_hor(model, prediction_horizon, horizon_length, iteration_count, x, z, w, g, s, all_days, loading_days):
    
    if (horizon_length*(iteration_count+1)+prediction_horizon) >= len(loading_days)-(prediction_horizon/2) and (horizon_length*(iteration_count+1)+prediction_horizon) < len(loading_days):
        horizon_ext = len(loading_days)-(horizon_length*(iteration_count+1)+prediction_horizon)
    else:
        horizon_ext = 0
    ...
    for var in model.getVars():
        ...
        if var_name[0] == 'x':
            ...
            # now looks like this: ['AD-7', 'DESCON_1', '28', 'ART_START', '63']
            if int(varName_list[2]) > horizon_length*(iteration_count+1)+prediction_horizon+horizon_ext:
            ...
            # now looks like this: [1001,6]
            if int(varName_list[1]) > horizon_length*(iteration_count+1)+prediction_horizon+horizon_ext:
    ...
    for tuple_key in x_vars_to_remove:
        model.remove(x[tuple_key])
        del x[tuple_key]

    for tuple_key in z_vars_to_remove:
        model.remove(z[tuple_key])
        del z[tuple_key]

    for tuple_key in w_vars_to_remove:
        model.remove(w[tuple_key])
        del w[tuple_key]

    for tuple_key in g_vars_to_remove:
        model.remove(g[tuple_key])
        del g[tuple_key]

    for tuple_key in s_vars_to_remove:
...
def add_variables_for_next_main_horizon(model, x, z, w, g, s, prediction_horizon, horizon_length, total_feasible_arcs, 
    ...
    start_time = time.time()
    print("\n--- Starting to add var for next horizon: %.1f seconds ---" % (time.time() - start_time))
    
        #total_feasible_arcs_for_x_generation = total_feasible_arcs

    new_feasible_arcs_for_x_generation = [(v, i, t, j, t_) for (v, i, t, j, t_) in total_feasible_arcs if (horizon_length*(iteration_count+1)) <= int(t) <= (horizon_length*(iteration_count+2)+horizon_ext+overlap)]
    
    x_new = model.addVars(new_feasible_arcs_for_x_generation, vtype = GRB.BINARY, name = 'x')
    ...
    new_fob_dimensions = [(f,t) for f in fob_ids for t in fob_days[f] if (horizon_length*(iteration_count+1)) <= int(t) <= (horizon_length*(iteration_count+2)+horizon_ext+overlap)] # Each fob contract has a specific loading node 

    z_new = model.addVars(new_fob_dimensions, vtype = GRB.BINARY, name='z')

    model.update()

    z.update(z_new)

    charter_dimensions = [(i,t,j) for j in (des_contract_ids + des_spot_ids) for i in des_loading_ports[j] for t in loading_days if (t+sailing_time_charter[i,j] in unloading_days[j]) and (horizon_length*(iteration_count+1)) <= int(t) <= (horizon_length*(iteration_count+2)+horizon_ext+overlap)]
    
    w_new = model.addVars(charter_dimensions, vtype = GRB.BINARY, name='w')
    ...
    g_new = model.addVars(charter_dimensions, vtype='C', name='g')
    ...
    production_quantities = {(i,t): value for (i,t), value in production_quantities.items() if (horizon_length*(iteration_count+1)) <= int(t) <= (horizon_length*(iteration_count+2)+horizon_ext+overlap)}

    s_new = model.addVars(production_quantities, vtype='C', name='s')
    ...
    model.update()
    print("\n--- Done adding var for next horizon: %.1f seconds ---" % (time.time() - start_time))

    return model, x, z, w, g, s


def freeze_variables(model, x, z, w, g, s, horizon_length, iteration_count, prediction_horizon, all_days, horizon_ext, overlap):
    ...
    for var in model.getVars():
        ...
        if var_name[0] == 'x':
            ...
            #elif horizon_length*(iteration_count+1) <= int(varName_list[2]) < horizon_length*(iteration_count+2)+horizon_ext:
            #    #var.vtype = GRB.BINARY
            #    x[tuple_key].vtype = GRB.BINARY
            # making the variables in the next prediction horizon continous ("ALL"):
            #elif horizon_length*(iteration_count+2)+overlap <= int(varName_list[2]) < horizon_stop_time:
            #    #var.vtype = GRB.CONTINUOUS
            #    x[tuple_key].vtype = GRB.CONTINUOUS
            '''
            else:
                model.remove(var)
                del x[var]+horizon_ext
            '''
        ...
        if var_name[0]=='g':
            ...
            if 0 <= int(varName_list[1]) < horizon_length*(iteration_count+1):
                #var.lb = var.X
                #var.ub = var.X
                if var.X > CHARTER_VESSEL_UPPER_CAPACITY:
                    g[tuple_key].lb = math.floor(var.X)
                    g[tuple_key].ub = math.floor(var.X)
                elif var.X < CHARTER_VESSEL_LOWER_CAPACITY and var.X >= 20:
                    g[tuple_key].lb = math.ceil(var.X)
                    g[tuple_key].ub = math.ceil(var.X)
                elif var.X < 20:
                    g[tuple_key].lb = 0
                    g[tuple_key].ub = 0
                else:
            ...
            # making the variables in the next horizon binary:
            #elif horizon_length*(iteration_count+1) <= int(varName_list[1]) < horizon_length*(iteration_count+2)+horizon_ext:
                #var.vtype = GRB.BINARY
            #    w[tuple_key].vtype = GRB.BINARY
            '''
            else:
                model.remove(var)
                del g[var]
        ...
        if var_name[0]=='z':
            ...
            # making the variables in the next prediction horizon continous ("ALL"):
            #elif horizon_length*(iteration_count+2)+overlap <= int(varName_list[1]) < horizon_stop_time:
            #    #var.vtype = GRB.CONTINUOUS
...
def add_variables_for_next_prediction_horizon(model, x, z, w, g, s, prediction_horizon, horizon_length, total_feasible_arcs, 
                                              iteration_count, fob_ids, fob_days, des_contract_ids, des_spot_ids, loading_port_ids, 
                                              loading_days, production_quantities, horizon_ext,sailing_time_charter, unloading_days, des_loading_ports, overlap):
    ...
    new_feasible_arcs_for_x_generation = [(v, i, t, j, t_) for (v, i, t, j, t_) in total_feasible_arcs if (horizon_length*(iteration_count+2)+overlap <= int(t) <= (horizon_length*(iteration_count+2)+prediction_horizon+horizon_ext))]
    
    x_new = model.addVars(new_feasible_arcs_for_x_generation, vtype = GRB.CONTINUOUS, name = 'x')
    ...
    new_fob_dimensions = [(f,t) for f in fob_ids for t in fob_days[f] if (horizon_length*(iteration_count+2)+overlap <= int(t) <= (horizon_length*(iteration_count+2)+prediction_horizon+horizon_ext))] # Each fob contract has a specific loading node 

    z_new = model.addVars(new_fob_dimensions, vtype = GRB.CONTINUOUS, name='z')
    ...
    charter_dimensions = [(i,t,j) for j in (des_contract_ids + des_spot_ids) for i in des_loading_ports[j] for t in loading_days if (t+sailing_time_charter[i,j] in unloading_days[j]) and (horizon_length*(iteration_count+2)+overlap <= int(t) <= (horizon_length*(iteration_count+2)+prediction_horizon+horizon_ext))]
    #charter_dimensions = [(i,t,j) for i in loading_port_ids for t in loading_days if (horizon_length*(iteration_count+2) <= int(t) <= (horizon_length*(iteration_count+2)+prediction_horizon+horizon_ext)) for j in (des_contract_ids + des_spot_ids)]
    
    w_new = model.addVars(charter_dimensions, vtype ='C', name='w')
    ...
    production_quantities = {(i,t): value for (i,t), value in production_quantities.items() if (horizon_length*(iteration_count+2)+overlap <= int(t) <= (horizon_length*(iteration_count+2)+prediction_horizon+horizon_ext))}
