...
def init_objective(leftover_stop_time, x, z, s, w, g, fob_revenues, fob_demands, fob_ids, fob_days, des_contract_revenues, vessel_capacities, vessel_boil_off_rate,
...
tank_leftover_value, vessel_available_days, des_contract_ids, sailing_costs, charter_total_cost, des_spot_ids):
    ...
    for f in fob_ids for t in fob_days[f] if (f,t) in z.keys()) + 
    ...
    if (v,i,t,j,t_) in x.keys() and (j,t_) in des_contract_revenues.keys()) + #her har vi kun generert x som har t i hor+pred_hor, så ingen edits trengs
    gp.quicksum(g[i,t,j]*(1-sailing_time_charter[i,j]*charter_boil_off)*des_contract_revenues[j,t+sailing_time_charter[i,j]] 
    for j in des_spot_ids for i in loading_port_ids for t in loading_days if (i,t,j) in g.keys() and (j,t+sailing_time_charter[i,j]) in des_contract_revenues.keys())
    + gp.quicksum(tank_leftover_value[i]*s[i, leftover_stop_time] for i in loading_port_ids) +
    ...
    if (v,i,t,j,t_) in x.keys() and (j,t+sailing_time_charter[i,j]) in des_contract_revenues.keys()) + 
    gp.quicksum(g[i,t,j]*(1-sailing_time_charter[i,j]*charter_boil_off)*des_contract_revenues[j,t+sailing_time_charter[i,j]] 
    for j in des_contract_ids for i in loading_port_ids for t in loading_days if (i,t,j) in g.keys() and (j,t+sailing_time_charter[i,j]) in des_contract_revenues.keys()) -
...
def init_initial_loading_inventory_constr(s, g, z, x, production_quantities, vessel_capacities, vessel_ids,
    ...
    fob_loading_ports, des_spot_ids):

    initial_loading_inventory_constraints = (s[i,t] == initial_inventory[i]+production_quantities[i,t]-gp.quicksum(vessel_capacities[v]*x[v,i,t,j,t_] 
...
def init_loading_inventory_constr(start_inventory_constraint_time, stop_time, s, g, z, x, production_quantities, vessel_capacities, vessel_ids,
    des_contract_ids, all_days,fob_demands, fob_ids, loading_port_ids, loading_days, fob_loading_ports, des_spot_ids, horizon_length, iteration_count):
    ...
    - gp.quicksum(fob_demands[f]*z[f,t] 
    for f in fob_ids if (f,t) in z.keys() and i in fob_loading_ports[f])
    for i in loading_port_ids for t in loading_days[start_inventory_constraint_time:stop_time])
...
def init_flow_constr(x, all_days, vessel_ids, port_ids, stop_time):
    ...
    for v in vessel_ids for j in port_ids for t in range(1,stop_time))
...
def init_upper_demand_constr(all_days_stop_time, x, g, vessel_capacities, vessel_boil_off_rate, vessel_ids, port_ids, loading_days,
    ...
    for v in vessel_ids for i in port_ids for t in loading_days for t_ in partition_days[p] if partition_days[p][-1] <= all_days_stop_time # Left-hand sums
    ...
    if t+sailing_time_charter[i,j] in partition_days[p] if partition_days[p][-1] <= all_days_stop_time if (i,t,j) in g.keys()) #relakserer dersom partisjonen går utenfor hor+pred_hor
    # only feasibl earcs
    <= upper_partition_demand[j,p] 
    for j in (des_contract_ids+des_spot_ids) for p in des_contract_partitions[j] if partition_days[p][-1] <= all_days_stop_time)

    return upper_demand_constraints


# Initialize lower demand constraints
def init_lower_demand_constr(all_days_stop_time, x, g, vessel_capacities, vessel_boil_off_rate, vessel_ids, port_ids, loading_days,
    ...
    if t+sailing_time_charter[i,j] in partition_days[p] if partition_days[p][-1] <= all_days_stop_time if (i,t,j) in g.keys())) # relaxing if partition goes outside hor+pred_hor+(all_days-loading_days)
...
def init_berth_constr(stop_time, x, z, w, vessel_ids, port_ids, loading_days, operational_times, des_contract_ids, fob_ids, fob_operational_times,
    ...
    + gp.quicksum(w[j,t_,j_] for j_ in (des_spot_ids + des_contract_ids) if (j,t_,j_) in w.keys())
